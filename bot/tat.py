...
def show_help(update: Update, context: CallbackContext):
    update.message.reply_text(
        "🕵️‍♂️ Spyfall — короткі правила:\n"
        "- Один із вас — шпигун і не знає локацію.\n"
        "- Інші — звичайні гравці, які знають місце.\n"
        "- Мета гравців — знайти шпигуна.\n"
        "- Мета шпигуна — не спалитися і вгадати локацію.\n\n"
        "Під час обговорення ставте один одному запитання. Потім буде голосування."
    )

def show_info(update: Update, context: CallbackContext):
    if not game_state['players']:
        update.message.reply_text("ℹ️ Інформація про гру недоступна.")
        return
    ...
    for pid, info in game_state['players'].items():
        name = info['name']
        if info['role'] == 'spy':
            text += f"- {name}: 🕵️ *Шпигун*\n"
        else:
            location = info.get('location', 'невідомо')
